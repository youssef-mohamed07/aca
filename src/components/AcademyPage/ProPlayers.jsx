...
import Container from "../../components/AcademyPage/Container";
...
const ProPlayers = ({ rectangle7, id, name, age, bio }) => {
  const navigate = useNavigate();
  const [data, setData] = useState([])
useEffect(() => {
  axios.get(`https://cocacademy-com.preview-domain.com/api/proPlayers`)
  .then(response => {
    setData(response.data);
localStorage.setItem('proPlayer', JSON.stringify(response.data));
